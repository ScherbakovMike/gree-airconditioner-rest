...
public class GreeAirconditionerDeviceFinder {
  ...
  public static List<GreeAirconditionerDevice> findDevices(InetAddress broadcastAddress) {
    if (log.isInfoEnabled()) {
      log.info(
          "Looking for air conditioner devices on {}. This might take a while...",
    ...
    DatagramPacket sendPacket =
        new DatagramPacket(scanCommand, scanCommand.length, broadcastAddress, 7000);
    try {
  ...
  private static CommandResponse getResponseCommand(DatagramPacket receivePacket)
      throws IOException {
