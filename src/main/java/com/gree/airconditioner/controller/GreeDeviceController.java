...
import com.gree.airconditioner.service.GreeDeviceManager;
...
@Slf4j
@Tag(name = "GREE Air Conditioner API", description = "REST API for managing GREE air conditioning devices")
public class GreeDeviceController {
    
    private final GreeDeviceManager deviceManager;
    
    @GetMapping("/discover")
    @Operation(summary = "Discover GREE devices", description = "Scan the network to discover available GREE air conditioning devices")
    public CompletableFuture<ResponseEntity<ApiResponse<List<DeviceInfoDto>>>> discoverDevices() {
        log.info("Starting device discovery...");
        return deviceManager.discoverDevices()
                .thenApply(devices -> {
                    log.info("Device discovery completed. Found {} devices", devices.size());
                    return ResponseEntity.ok(ApiResponse.success("Devices discovered successfully", devices));
                })
                .exceptionally(ex -> {
                    log.error("Error during device discovery", ex);
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                            .body(ApiResponse.<List<DeviceInfoDto>>error("Failed to discover devices: " + ex.getMessage()));
                });
    }
    ...
    public ResponseEntity<ApiResponse<List<DeviceInfoDto>>> getAllDevices() {
        try {
            List<DeviceInfoDto> devices = deviceManager.getDevices();
            log.info("Retrieved {} devices", devices.size());
            return ResponseEntity.ok(ApiResponse.success(devices));
        } catch (Exception e) {
            log.error("Error retrieving devices", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.<List<DeviceInfoDto>>error("Failed to retrieve devices: " + e.getMessage()));
        }
    }
    
    @PostMapping("/{deviceId}/connect")
    @Operation(summary = "Connect to device", description = "Establish connection to a specific GREE device")
    public CompletableFuture<ResponseEntity<ApiResponse<String>>> connectToDevice(@PathVariable String deviceId) {
        log.info("Connecting to device: {}", deviceId);
        return deviceManager.connectToDevice(deviceId)
                .thenApply(success -> {
                    if (success) {
                        log.info("Successfully connected to device: {}", deviceId);
                        return ResponseEntity.ok(ApiResponse.success("Connected to device successfully", "Connected"));
                    } else {
    ...
    @Operation(summary = "Disconnect from device", description = "Disconnect from a specific GREE device")
    public CompletableFuture<ResponseEntity<ApiResponse<String>>> disconnectFromDevice(@PathVariable String deviceId) {
        log.info("Disconnecting from device: {}", deviceId);
        return deviceManager.disconnectFromDevice(deviceId)
                .thenApply(success -> {
                    if (success) {
                        log.info("Successfully disconnected from device: {}", deviceId);
                        return ResponseEntity.ok(ApiResponse.success("Disconnected from device successfully", "Disconnected"));
                    } else {
    ...
    @Operation(summary = "Get device status", description = "Get current status and properties of a GREE device")
    public CompletableFuture<ResponseEntity<ApiResponse<DeviceStatusDto>>> getDeviceStatus(@PathVariable String deviceId) {
        ...
        return deviceManager.getDeviceStatus(deviceId)
                .thenApply(status -> {
                    ...
                    return ResponseEntity.ok(ApiResponse.success("Device status retrieved successfully", status));
                })
                .exceptionally(ex -> {
                    log.error("Error getting status for device {}", deviceId, ex);
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                            .body(ApiResponse.<DeviceStatusDto>error("Failed to get device status: " + ex.getMessage()));
                });
    }
    ...
    public CompletableFuture<ResponseEntity<ApiResponse<String>>> controlDevice(
        ...
        return deviceManager.controlDevice(deviceId, controlDto)
                .thenApply(success -> {
                    if (success) {
                        log.info("Successfully controlled device: {}", deviceId);
                        return ResponseEntity.ok(ApiResponse.success("Device controlled successfully", "Success"));
                    } else {
        ...
        if (temperature < 16 || temperature > 30) {
            return CompletableFuture.completedFuture(
                    ResponseEntity.badRequest()
                            .body(ApiResponse.<String>error("Temperature must be between 16 and 30 degrees Celsius"))
            );
        }
    ...
    @Operation(summary = "Set mode", description = "Set operating mode (AUTO, COOL, HEAT, DRY, FAN_ONLY)")
